...
export default function EditEvent() {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'totalTickets') {
      ...
      // Calculate booked tickets safely:
      // If remaining === total, then booked = 0 (no bookings)
      const bookedTickets = (event.remainingTickets === event.totalTickets)
        ? 0
        : event.totalTickets - event.remainingTickets;

      if (newTotal < bookedTickets) {
        alert(`Cannot set total tickets below already booked tickets (${bookedTickets}).`);
        return; // ignore invalid input
      }

      const newRemaining = newTotal - bookedTickets;
